...
This is the usual anatomy of a "false positive": a true finding about a latent fragility, mis-scoped as an active bug because the reviewer only sees the diff. The reviewer was wrong about *today* and right about *the function*.
...
The "false positive" produced a real improvement. The author never had to win the argument, because they never had it.
...
Not author fiat — still. The escape hatch is an **independent second opinion**, per [PROTOCOL.md §9.4](../PROTOCOL.md) (cross-model dispatch): send the same work product and the disputed verdict to a different model.
...
- If the second reviewer withdraws it after seeing the context, you have evidence the finding was model-specific — and the kind of artifact (position bias, a misread) the [Prior art](../README.md#prior-art) literature predicts. You ship on the second verdict, and you have a record of *why*.
...
- **The one move the protocol forbids is the one instinct demands:** ignoring a Critical because you *know* it's wrong. That is precisely the discretion-creep the floor exists to remove.
